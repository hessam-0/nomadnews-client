...
const getArticles = (topic, sort_by, order) => {
  return api.get("/articles", { params: { topic, sort_by, order } }).then((response) => {
...
const patchArticleVotes = (article_id, inc_votes) => {
  return api.patch(`/articles/${article_id}`, { inc_votes }).then((response) => {
    return response.data.article;
  });
};

const patchCommentVotes = (comment_id, inc_votes) => {
  return api.patch(`/comments/${comment_id}`, { inc_votes }).then((response) => {
